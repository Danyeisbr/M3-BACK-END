...
/*   async.each(['poem-two/stanza-01.txt', 'poem-two/stanza-02.txt'],
  ...
  // AsyncAwait version
    let poemas = ['poem-two/stanza-01.txt', 'poem-two/stanza-02.txt'];

    async function readFile (file){
      blue(await promisifiedReadFile(file))
    }

    await Promise.all(poemas.map(stanza => readFile(stanza)))
...
/*   async.each(filenames,
  ...
  await Promise.all(filenames.map(stanza => readFile(stanza)))
...
async function problemC () {
  ...
  const results = await Promise.all(filenames.map(file => promisifiedReadFile(file)))
  results.forEach((stanza)=> blue(stanza))
  console.log("done")
